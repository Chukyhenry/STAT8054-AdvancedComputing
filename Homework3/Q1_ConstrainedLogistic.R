...
# Accelerated Projected Gradient Descent for Constrained Logistic Regression
LinCon_LogisticReg <- function(X, y, A, b, alpha, max_iter, tol ) {
  ...
  for (iteration in 1:max_iter) {
    grad <- gradient(beta)
    beta_new <- project(beta - alpha * grad)
...
# Plot convergence
plot(out$objective_values, type = "l", main = "Objective Function Convergence",
